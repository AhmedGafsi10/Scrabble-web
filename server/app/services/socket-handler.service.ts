...
export class SocketHandlerService {
    ...
    /*  handleJoinRoomSoloBot(socket: io.Socket, data: { roomName: string; botName: string; isExpertLevel: boolean }, wordFetcher: WordFetcher) {
        ...
        room.createPlayerVirtual(socket.id, data.botName, wordFetcher, desiredLevel);
    ...
    handleConvertToRoomSoloBot(socket: io.Socket, data: { roomName: string; botName: string; points: number; isExpertLevel: boolean }) {
        ...
        this.roomService.setUnavailable(room.roomInfo.name);
        this.sendToEveryone('updateAvailableRoom', this.roomService.getRoomsAvailable());
        // TODO: Add bot greeting
    }

    // roomName include the socketId
    ...
    handleDisconnecting(socket: io.Socket) {
        ...
        if (room.players.length <= 1) {
            ...
            setTimeout(() => {
                ...
                if (!player) return;
                if (room.bot) {
                    // TODO: it should not update score anymore if we give up
    ...
    getSocketRoom(socket: io.Socket) {
