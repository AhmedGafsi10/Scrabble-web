...
describe('BotService', () => {
    ...
    describe('deleteAllBots() tests', () => {
        ...
        /* it('should remove all of the bots without the title DEFAULT_BOT_NAME', async () => {
            await botService.collection.insertOne(defaultBot);
            await botService.collection.insertOne(bot1);
            await botService.collection.insertOne(bot2);
            await botService.deleteAllBots();
            const bots = await botService.collection.find({}, { projection: { _id: 0 } }).toArray();
            expect(bots.length).to.equal(1);
            expect(bots[0].name).to.deep.equals(defaultBot.name);
            expect(bots[0].name).to.deep.equals(DEFAULT_BOT_NAME);
            expect(bots[0].gameType).to.deep.equals(defaultBot.gameType);
        });
